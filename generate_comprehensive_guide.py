...
def generate():
    ...
    doc.add_callout(
        "این جزوه خودآموز جامع برای این تدوین شده است تا شما بدون نیاز به داشتن دانش عمیق قبلی در ریاضیات پیشرفته یا یادگیری عمیق، به کل متن، مفاهیم، فرمول‌ها، نتایج عددی، اصطلاحات و منطق علمی پایان‌نامه خود کاملاً مسلط شوید و بتوانید با آرامش و تسلط کامل در جلسه دفاع حاضر شده و نمره عالی کسب کنید.",
    ...
    doc.add_bullet("توسعه حل‌گر مرجع طیفی فوریه: طراحی یک حل‌گر فوق‌العاده دقیق کلاسیک بر پایه بسط فوریه که صرفاً به عنوان «خط‌کش آزمون و پاسخ مرجع دقیق» برای محاسبه خطای شبکه استفاده شده است.", "حل‌گر مرجع:")
    ...
    doc.add_paragraph("این فصل مهم‌ترین بخش پایان‌نامه شماست و شامل دو محور اساسی است:", bold=True, color='#003366')
    ...
    doc.add_bullet("بستر محاسباتی (بخش ۴-۱): استفاده از پایتون ۳ و کتابخانه JAX با کامپایلر در زمان اجرا (JIT) و بردارسازی خودکار (VMAP) برای پردازش موازی و سریع.", "بستر نرم‌افزاری:")
    doc.add_bullet("حل‌گر مرجع طیفی (بخش ۴-۲): پیاده‌سازی حل‌گر فوریه با روش گام‌زنی زمانی نمایی مرتبه چهار (ETDRK4) روی شبکه ۲۵۶ نقطه مکانی و ۱۰۰ مقطع زمانی و راستی‌آزمایی آن با جواب نیمه‌تحلیلی کول-هوپف.", "حل‌گر مرجع:")
    ...
    num_data = [
        ...
        ["خطای نسبی L2 نهایی", "2.15 * 10^-2  (معادل ۲.۱۵ درصد)", "میزان انحراف شبکه از حل مرجع طیفی دقیق"],
    ...
    doc.add_heading1("۴. تشریح دقیق نمودارهای فصل ۴ و مکانیزم ۵ مرحله‌ای AD")
    
    doc.add_heading2("تحلیل مفهومی شکل‌ها و نمودارهای فصل ۴:")
    doc.add_bullet("شکل ۴-۱ (توزیع نقاط آموزش): این شکل نشان می‌دهد که چگونه ۲۰۰ نقطه با رنگ آبی روی مرزهای مکانی و لحظه آغازین قرار گرفته‌اند و ۱۰۰۰۰ نقطه خاکستری در سراسر صفحه فضا-زمان پخش شده‌اند. تراکم بالاتر نقاط در خط مرکزی x=0 نشان‌دهنده راهبرد نمونه‌برداری متمرکز در ناحیه تشکیل شوک است.", "شکل ۴-۱:")
    ...
    doc.add_bullet("شکل ۴-۳ (مقاطع زمانی پاسخ در زمان‌های t=0.25, 0.50, 0.75): این نمودار قلب نتایج شماست! خطوط ممتد قرمز پیش‌بینی PINN و دایره‌های آبی حل مرجع طیفی هستند. مشاهده می‌کنید که موج سینوسی هموار اولیه با گذر زمان به سمت مرکز متمایل شده و در حوالی x=0 یک افت عمودی و دیوارمانند (جبهه شوک) ایجاد می‌کند و مدل با دقت عالی روی حل مرجع منطبق است.", "شکل ۴-۳:")
    doc.add_bullet("شکل ۴-۴ (نقشه خطای دوبعدی): نمودار کانتور رنگی نشان می‌دهد که در بیش از ۹۵ درصد مساحت دامنه فضا-زمان، خطا به رنگ آبی تیره (نزدیک به صفر) است و بیشینه خطای اندک مدل فقط در امتداد نوار بسیار باریک خط شوک (x=0 برای زمان‌های بعد از 0.3) تمرکز دارد.", "شکل ۴-۴:")

    doc.add_heading2("مکانیزم ۵ گانه مشتق‌گیری خودکار در حل برگرز (اگر داور گفت فرمول‌ها را بنویس):")
    
    ad_steps = [
        ("گام اول (نگاشت شبکه):", "شبکه عصبی سرعت را با فرمول u_theta(t,x) = W_4 * h_3 + b_4 مدل می‌کند که در آن لایه‌ها از فرمول h_l = tanh(W_l * h_{l-1} + b_l) پیروی می‌کنند."),
        ("گام دوم (قاعده زنجیره‌ای):", "مشتقات جزئی با ضرب ژاکوبی‌ها روی گراف استخراج می‌شوند: مشتق زمانی u_t = d(u)/dt، مشتق مکانی u_x = d(u)/dx و مشتق مکانی دوم u_xx = d(u_x)/dx."),
        ("گام سوم (باقیمانده فیزیک):", "باقیمانده دیفرانسیلی فیزیک به صورت f_theta(t,x) = u_t + u*u_x - nu*u_xx تعریف می‌شود که در صورت درستی مدل، مقدار آن در تمام نقاط صفر است."),
        ("گام چهارم (تابع هزینه کل):", "تابع خطای کل برابر است با: Loss = MSE_u + MSE_f که در آن MSE_u خطای داده‌های مرزی و MSE_f میانگین مربعات باقیمانده فیزیک روی نقاط هم‌مکانی است."),
        ("گام پنجم (به‌روزرسانی وزن‌ها):", "گرادیان تابع هزینه نسبت به وزن‌های شبکه (grad_theta Loss) با پس‌انتشار محاسبه شده و وزن‌ها با الگوریتم‌های Adam و L-BFGS آپدیت می‌شوند.")
    ]
    ...
    terms_data = [
        ...
        ["مسئله مستقیم (Forward Problem)", "مسئله‌ای که ساختار معادله و شرایط مرزی-اولیه معلوم است و هدف محاسبه میدان پاسخزمانی-مکانی u(t,x) است."],
        ...
        ["تابع فعال‌ساز تانژانت هیپربولیک", "تابع غیرخطی کراندار و بی‌نهایت‌بار مشتق‌پذیر (C-infinity) که مانع از صفر شدن مشتق مرتبه دوم (u_xx) می‌گردد."],
        ...
        ["حل‌گر طیفی فوریه (Spectral Method)", "روش عددی بسیار دقیق بر پایه تجزیه دامنه به توابع مثلثاتی فوریه با نرخ همگرایی نمایی که به عنوان حل دقیق مرجع به کار رفت."]
    ]
    ...
    doc.add_qa_box(
        "سؤال ۵: چرا حل‌گر مرجع طیفی فوریه پیاده‌سازی کردید؟ مگر هدف استفاده از PINN نبود؟",
        "پاسخ شما: معادله غیرخطی برگرز با لزجت دارای جواب تحلیلی بسته و ساده نیست (جواب نیمه‌تحلیلی کول-هوپف نیز نیازمند انتگرال‌گیری‌های عددی پیچیده است). برای اینکه بتوانیم درستی کار شبکه عصبی را به صورت کمی بسنجیم، خطای نسبی در نرم L2 را گزارش کنیم و نقشه خطای دوبعدی رسم نماییم، نیازمند یک «خط‌کش آزمون با دقت فوق‌العاده بالا» بودیم. بنابراین یک حل‌گر طیفی فوریه مستقل با روش زمانی ETDRK4 نوشتیم تا نتایج PINN را با آن اعتبارسنجی کنیم."
    )
    ...
    slide_data = [
        ...
        ["اسلاید ۷ و ۸\n(زمان: ۳ دقیقه)", "تابع هزینه دوقلو و آموزش ترکیبی", "تشریح تابع خطای Loss = MSE_u + MSE_f با ۲۰۰ نقطه مرزی و ۱۰۰۰۰ نقطه هم‌مکانی؛ توضیح استراتژی آموزش دو مرحله‌ای (Adam برای فاز اول و L-BFGS برای فاز نهایی)."],
        ["اسلاید ۹ و ۱۰\n(زمان: ۲ دقیقه)", "حل‌گر مرجع طیفی فوریه", "توضیح توسعه حل‌گر طیفی با روش زمانی ETDRK4 به عنوان خط‌کش آزمون دقیق برای سنجش صحت خروجی شبکه عصبی."],
        ["اسلاید ۱۱ تا ۱۳\n(زمان: ۵ دقیقه)", "نتایج تجربی، نمودارها و مقایسه", "نمایش نمودار مقاطع زمانی شکل ۴-۳ و بازسازی دقیق شیب شوک؛ نمایش نقشه خطای شکل ۴-۴؛ گزارش خطای نسبی ۲.۱۵ درصد و مقایسه تطبیقی با مقاله رئیسی ۲۰۱۹."],
    ...
    doc.add_callout(
        "«شبکه عصبی آگاه از فیزیک (PINN) یک حل‌گر عددی بدون شبکه محاسباتی است که با استفاده از مشتق‌گیری خودکار، قوانین بنیادین فیزیک را در کل دامنه فضا-زمان ارضا کرده و با کمترین داده‌های مرزی، رفتار پیچیده امواج شوک را با دقت بالا بازسازی می‌نماید.» — با آرزوی موفقیت، درخشش و نمره ۲۰ برای شما!",
